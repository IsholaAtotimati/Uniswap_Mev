...
async function main() {
    ...
    const signer = new ethers.Wallet(env.PRIVATE_KEY).address;
    const payloadBuilder = new PayloadBuilder();
    const signatureService = new SignatureService(env.PRIVATE_KEY);
    ...
    const recovered = signatureService.verify(payload, signature);

    console.log("===== Signed LossPayload =====");
