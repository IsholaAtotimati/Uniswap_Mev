...
export interface FeatureVector {
    ...
    walletFrequency: number;

    recentPoolVolumeUSD: number;

    uniqueWallets24h: number;

    pendingTxDensity: number;
}

export class FeatureExtractor {
    async extract(context: SwapContext): Promise<FeatureVector> {
        const liquidityUSD = Number(context.liquidity) / 1e18;

        const swapValueUSD = Math.abs(Number(context.amount0)) / 1e18;

        const utilization =
            swapValueUSD / Math.max(liquidityUSD, 1);
