...
export class SettlementEventListener {
     ...
     */
    onSettlementAuthorized(callback) {
    ...
    async queryHistoricalEvents() {
        ...
        try {
            ...
            const events = await this.contract.queryFilter(this.contract.filters.SettlementAuthorized(), fromBlock, currentBlock);
            logger.info({
 ...
 */
export function loadHookAbi(abiPath) {
    try {
        if (!abiPath) {
