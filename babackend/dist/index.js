...
import { SettlementStatus } from "./services/SettlementService.js";
...
async function bootstrap() {
    ...
    if (env.HOOK_ADDRESS && env.HOOK_ADDRESS !== "0x") {
        ...
        settlementListener.onSettlementAuthorized(async (event) => {
            ...
            if (existing) {
                settlementService.updateStatus(event.settlementId, SettlementStatus.Submitted, event.transactionHash);
            }
            else {
                settlementService.create(event.settlementId, "unknown", 0, event.token, Number(event.amount), event.destinationDomain, "0x0000000000000000000000000000000000000000");
                settlementService.updateStatus(event.settlementId, SettlementStatus.Submitted, event.transactionHash);
            }
