...
function buildPoolKey(tokenIn, tokenOut) {
    return {
        currency0: tokenIn,
        currency1: tokenOut,
        fee: 3000,
...
function buildPoolId(tokenIn, tokenOut) {
...
app.post("/api/execution-intent", async (req, res) => {
    try {
        ...
        // 2. Build payload
        const derivedPoolId = buildPoolId(context.token0, context.token1);
        // 2. Build payload
        const riskPayload = payloadBuilder.build(riskResult, derivedPoolId, signerAddress);
...
app.post("/swap/analyze", async (req, res) => {
    try {
        ...
        const { userWallet, poolId, tokenIn, tokenOut, amountIn, recipient, destinationDomain = 0 } = req.body;
        ...
        let settlement = settlementService.create(settlementId, derivedPoolId, payload.nonce, settlementToken, settlementAmount, destinationDomain, recipient, {
            asset: "USDC",
        ...
        if (hookClient) {
            try {
                ...
                const updated = settlementService.updateStatus(settlement.settlementId, SettlementStatus.Failed, undefined, { lastError: errorMessage });
                if (updated)
                    settlement = updated;
